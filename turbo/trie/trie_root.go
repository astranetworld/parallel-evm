...
import (
	...
	"starlink-world/erigon-evm/turbo/rlphacks"
)
...
func UnmarshalTrieNodeTyped(v []byte) (hasState, hasTree, hasHash uint16, hashes []libcommon.Hash, rootHash libcommon.Hash) {
	...
	hashes = make([]libcommon.Hash, len(v)/length.Hash)
...
func CastTrieNodeValue(hashes, rootHash []byte) []libcommon.Hash {
	to := make([]libcommon.Hash, len(hashes)/length.Hash+len(rootHash)/length.Hash)
	i := 0
	if len(rootHash) > 0 {
