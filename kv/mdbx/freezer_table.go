...
// NewFreezerTable opens the given path as a baseFreezer table.
...
func newTable(path string, name string, maxFilesize uint32, noCompression, readonly bool) (*freezerTable, error) {
	...
	if readonly {
		...
		// the additional meta file for each baseFreezer table is added in order to support
	...
	if _, err := tab.sizeNolock(); err != nil {
		tab.Close()
		return nil, err
	}
...
func (t *freezerTable) repair() error {
	...
	// Read the last index, use the default value in case the baseFreezer is empty
	...
	for contentExp != contentSize {
		...
		if contentExp > contentSize {
			...
			// the baseFreezer is already empty.
	...
	t.logger.Debug("Chain baseFreezer table opened", "items", t.items, "size", common.StorageSize(t.headBytes))
	return nil
}
...
func (t *freezerTable) truncateHead(items uint64) error {
	...
	log("Truncating baseFreezer table", "items", existing, "limit", items)

	// Truncate the index file first, the tail position is also considered
	// when calculating the new baseFreezer table length.
...
func (t *freezerTable) getIndices(from, count uint64) ([]*indexEntry, error) {
	...
	if from == 0 {
		// Special case if we're reading the first item in the baseFreezer. We assume that
...
// be used to analyse a live baseFreezer table index.
...
func (t *freezerTable) dumpIndex(w io.Writer, start, stop int64) {
	...
	if err != nil {
		fmt.Fprintf(w, "Failed to decode baseFreezer table %v\n", err)
		return
	}
