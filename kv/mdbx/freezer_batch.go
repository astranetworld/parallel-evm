...
func (batch *freezerBatch) AppendBytes(kind string, num uint64, data []byte) error {
	return batch.tables[kind].AppendBytes(num, data)
}
...
func (batch *freezerBatch) hasData() bool {
	for _, tb := range batch.tables {
		if tb.totalBytes > 0 {
			return true
		}
...
// Append rlp-encodes and adds data at the end of the baseFreezer table. The item number is a
...
func (batch *freezerTableBatch) AppendBytes(item uint64, data []byte) error {
	if item != batch.curItem {
		return fmt.Errorf("%w: have %d want %d", errOutOrderInsertion, item, batch.curItem)
	}
...
// AppendRaw injects a binary blob at the end of the baseFreezer table. The item number is a
// precautionary parameter to ensure data correctness, but the table will reject already
// existing data.
