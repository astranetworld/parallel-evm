...
func (n *InternalNode) Insert(key []byte, value []byte, resolver NodeResolverFn) error {
	...
	case *HashedNode:
		...
		hash := child.ComputeCommitmentBytes()
		serialized, err := resolver(hash[:])
		if err != nil {
			return fmt.Errorf("verkle tree: error resolving node %x at depth %d: %w", key, n.depth, err)
		}
		if len(serialized) == 0 {
			return fmt.Errorf("verkle tree: nil resolving node %x at depth %d: %w", key, n.depth, err)
		}
	...
	case *LeafNode:
		// Need to add a new branch nodein to differentiate
...
func (n *InternalNode) toHashedNode() *HashedNode {
	...
	return NewHashedNode(&hash, n.commitment)
}
...
func (n *InternalNode) ComputeCommitmentBytes() []byte {
	if n.commit != nil {
		return n.commit
	}
	n.ComputeCommitment()
...
func (n *InternalNode) ComputeCommitment() *Point {
	...
	for idx, child := range n.children {
		...
		default:
			...
			go func(idx int) {
				defer s.Done()
				toFr(&poly[idx], n.children[idx].ComputeCommitment())
			}(ii)
		}
	}
	s.Wait()
...
func (n *InternalNode) Serialize() ([]byte, error) {
	...
	for i, c := range n.children {
		if _, ok := c.(Empty); !ok {
			setBit(bitlist[:], i)
			digits := c.ComputeCommitmentBytes()
...
func (n *LeafNode) ComputeCommitmentBytes() []byte {
	if n.commit != nil {
		return n.commit
	}
	n.ComputeCommitment()
