...
func NewHashedNode(hash *Fr, commitment *Point) *HashedNode {
	b := commitment.Bytes()
	return &HashedNode{hash: hash, commitment: commitment, commit: b[:]}
}
...
func (n *HashedNode) ComputeCommitmentBytes() []byte {
	return n.commit
}
...
func (n *HashedNode) Copy() VerkleNode {
	...
	if n.commitment != nil {
		...
		h.commit = make([]byte, len(n.commit))
		copy(h.commit, n.commit)
	}
