...
var (
	...
	experiments                    []string
	chainName                      string // Which chain to use (mainnet, ropsten, rinkeby, goerli, etc.)
	syncmodeStr                    string
)
...
func main() {
	...
	db := openDB(dirs.Chaindata, logger, true)
	defer db.Close()
	ctx, _ := RootContext()
	var batchSize datasize.ByteSize
	must(batchSize.UnmarshalText([]byte(batchSizeStr)))
	pm, engine, chainConfig, vmConfig := newSync(ctx, db, nil)
	cfg := stage.StageExecuteBlocksCfg(db, pm, batchSize, nil, chainConfig, engine, vmConfig, nil, false, false, false, dirs, getBlockReader(db), nil, nil, ethconfig.Sync{}, nil)
	//go func() {
