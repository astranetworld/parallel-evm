...
import (
	...
	"starlink-world/erigon-evm/common/u256"
...
type IntraBlockState struct {
	...
	//todo 用于记录区块执行时需要准备哪些账户,storage k/v.这样用于在手机上执行.
	snap *Snapshot
	//todo 用于记录生成手机上的完整数据时记录需要准备哪些合约,方便查询
	codeMap     map[libcommon.Hash][]byte
...
// 打开开关,开始记录执行该消息需要准备的账户,storage keyvalue.
// 如果不打开这个开关就不会记录相应的数据.
// 用于在手机端验证.
...
// 打开开关,开始记录执行该消息需要准备的合约,可能包含第三方合约
// 如果不打开这个开关就不会记录相应的合约.
// 用于在手机端验证.
func (sdb *IntraBlockState) BeginWriteCodes() {
	sdb.codeMap = make(map[libcommon.Hash][]byte, 0)
}

// 返回记录相应的合约.
func (sdb *IntraBlockState) CodeHashes() map[libcommon.Hash][]byte {
	return sdb.codeMap
}

// 设置当前区块号
func (sdb *IntraBlockState) SetHeight(height uint32) {
	sdb.height = height
}
...
// 设置使用快照时区块的输出的哈希..
func (sdb *IntraBlockState) SetOutHash(hash libcommon.Hash) {
...
// 设置使用快照时区块的输出的哈希,并返回快照的rlp编码
func (sdb *IntraBlockState) WrittenSnapshot(hash libcommon.Hash) []byte {
	if sdb.snap == nil || !sdb.snap.CanWrite() {
		return nil
	}
...
// 设置快照查找不到时通过f1去查找相应的数据
func (s *IntraBlockState) SetGetOneFun(f1 GetOneFun) {
	s.snap.SetGetFun(f1)
}

// 从快照rlp编码中解析出快照并设置为当前的快照,用于手机端验证.
...
func (sdb *IntraBlockState) CheckReadableSnap(h libcommon.Hash) bool {
	if sdb.snap == nil || sdb.snap.CanWrite() {
		return true
	}
	return sdb.snap.OutHash == h
}
...
func (sdb *IntraBlockState) GetCode(addr libcommon.Address) []byte {
	...
	if stateObject != nil && !stateObject.deleted {
		...
		//如果要记录当前区块的合约使用信息,需要更新.
		if len(code) > 0 && sdb.codeMap != nil {
			sdb.codeMap[stateObject.data.CodeHash] = code
		}
		return code
	}
	if sdb.trace {
...
func (sdb *IntraBlockState) GetCodeSize(addr libcommon.Address) int {
	...
	//如果要记录当前区块的合约使用信息,需要更新.
	if l > 0 && sdb.codeMap != nil {
		code, err := sdb.stateReader.ReadAccountCode(addr, stateObject.data.Incarnation, libcommon.BytesToHash(stateObject.CodeHash()))
		if err != nil {
			sdb.setErrorUnsafe(err)
		}
		sdb.codeMap[stateObject.data.CodeHash] = code
...
func (sdb *IntraBlockState) getStateObject(addr libcommon.Address) (stateObject *stateObject) {
	...
	//如果只读snap不为空,则从snap中读取相应的账户信息
	if sdb.snap != nil && !sdb.snap.CanWrite() {
		account, err := sdb.snap.ReadAccountData(addr)
		if err != nil {
			sdb.setErrorUnsafe(err)
			return nil
		}
		if account == nil {
			//sdb.stateReader.ReadAccountData(addr)
