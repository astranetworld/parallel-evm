...
import (
	...
	"starlink-world/erigon-evm/common/changeset"
	...
	"starlink-world/erigon-evm/ethdb/bitmapdb"
	"starlink-world/erigon-evm/turbo/trie"
)
...
func TestMutationDeleteTimestamp(t *testing.T) {
	...
	i := 0
	err := changeset.ForEach(tx, kv.AccountChangeSet, nil, func(blockN uint64, k, v []byte) error {
		i++
		return nil
	})
	if err != nil {
	...
	count := 0
	err = changeset.ForPrefix(tx, kv.StorageChangeSet, dbutils.EncodeBlockNumber(1), func(blockN uint64, k, v []byte) error {
		count++
		return nil
	})
	if err != nil {
...
func TestMutationCommit(t *testing.T) {
	...
	err := changeset.ForPrefix(tx, kv.AccountChangeSet, dbutils.EncodeBlockNumber(2), func(_ uint64, k, v []byte) error {
	...
	err = changeset.ForPrefix(tx, kv.StorageChangeSet, dbutils.EncodeBlockNumber(2), func(_ uint64, k, v []byte) error {
...
func TestWalkAsOfStatePlain(t *testing.T) {
	...
	for _, addr := range addrs {
		if err := WalkAsOfStorage(tx, addr, changeset.DefaultIncarnation, libcommon.Hash{}, 2, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
	...
	for _, addr := range addrs {
		if err := WalkAsOfStorage(tx, addr, changeset.DefaultIncarnation, libcommon.Hash{}, 4, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
	...
	for _, addr := range addrs {
		if err := WalkAsOfStorage(tx, addr, changeset.DefaultIncarnation, libcommon.Hash{}, 6, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
...
func TestWalkAsOfUsingFixedBytesStatePlain(t *testing.T) {
	...
	if err := WalkAsOfStorage(tx, addr1, changeset.DefaultIncarnation, libcommon.Hash{}, 2, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
		err := block2.Add(append(common.CopyBytes(kAddr), kLoc...), common.CopyBytes(v))
		if err != nil {
			t.Fatal(err)
		}
	...
	if err := WalkAsOfStorage(tx, addr1, changeset.DefaultIncarnation, libcommon.Hash{}, 4, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
		err := block4.Add(append(common.CopyBytes(kAddr), kLoc...), common.CopyBytes(v))
		if err != nil {
			t.Fatal(err)
		}
	...
	if err := WalkAsOfStorage(tx, addr1, changeset.DefaultIncarnation, libcommon.Hash{}, 6, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
		err := block6.Add(append(common.CopyBytes(kAddr), kLoc...), common.CopyBytes(v))
		if err != nil {
			t.Fatal(err)
		}
...
func TestWalkAsOfStoragePlain_WithChunks(t *testing.T) {
	...
	for _, blockNum := range []uint64{5, 100, 1000, 1050} {
		...
		for _, addr := range addrs {
			if err := WalkAsOfStorage(tx, addr, changeset.DefaultIncarnation, libcommon.Hash{}, blockNum, func(kAddr, kLoc []byte, v []byte) (b bool, e error) {
